...
class ConvoApp {
  ...
  bind(dialogFlowApp) {
    this.registeredIntents.forEach(({intent, intentHandler}) => {
      dialogFlowApp.intent(intent, (conv, params, extra) => {
        return intentHandler(new Convo(conv), params, extra);
      });
    });
  ...
  intent(convo, intent, params = {}, option = {}, debugOptions = {}){
    ...
    return registeredIntent.intentHandler(convo, params, option, debugOptions).then(convo => {return {app:this, convo:convo}});
  }
}
